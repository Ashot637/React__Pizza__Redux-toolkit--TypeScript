...
const SaladItem: FC<ISaladItemProps> = ({ id, title, img, sizes, price }) => {
    ...
    useEffect(() => {
        setItemPrice(price)
        const additionalSum = selectedSize * 0.7 * 60;
    ...
    return (
        <div className="item__block">
            <img src={img} style={{ width: 297 }} alt="" className="item__block__img" />
                        ...
                        sizes.map((size: number, i: number) => {
                            return (
                                <div key={i}
                                    ...
                                    onClick={() => onChangeSize(i)}>{size} г.</div>
                            )
                        })
                    }
                </div>
